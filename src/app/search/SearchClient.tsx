...
function SearchPageContent() {
  ...
  // "Near me" — real device location. Sorts every club by true distance from the
  // user; unlike the ZIP path we do NOT cap the radius, so players in thin markets
  // still see their closest options rather than an empty result set.
  const useMyLocation = () => {
  ...
  const filteredCourts = useMemo(() => {
    ...
    // Device-location proximity ("near me") — annotate distance, keep all clubs
    if (userCoords) {
    ...
    return results;
  }, [allCourts, searchQuery, selectedState, selectedCity, minRating, courtType, sortBy, zipSearchCoords, userCoords]);
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <Button
                    ...
                    variant={geoStatus === "on" ? "default" : "outline"}
                    className="w-full"
                    disabled={geoStatus === "locating"}
                  >
                    <Navigation className="w-4 h-4 mr-2" />
                    {geoStatus === "locating" ? "Finding you…" : geoStatus === "on" ? "Sorted by distance from you" : "Use my location"}
                  </Button>
                  {geoStatus === "denied" && (
                    <p className="text-xs text-muted-foreground">Location access was blocked — search by city or ZIP instead.</p>
                  )}
                  {geoStatus === "unsupported" && (
                    <p className="text-xs text-muted-foreground">This browser doesn&apos;t support location — search by city or ZIP instead.</p>
                  )}
                </div>
